   ...
   L.Control.Layers.Tree = L.Control.Layers.extend({
      ...
      expandSelected: function(overlay) {
         function iter(el) {
            ...
            if (p) {
               if (L.DomUtil.hasClass(p, that.cls.children) && !L.DomUtil.hasClass(el, that.cls.childrenNopad)) {
      ...
      _applyOnTree: function(container, collapse) {
         ...
         iters.forEach(function(it) {
            ...
            for (var i = 0; i < els.length; i++) {
               var el = els[i];
               if (L.DomUtil.hasClass(el, this.cls.childrenNopad)) {} else if (it.hide) {
      ...
      _iterateTreeLayout: function(tree, container, overlay, selAllNodes, noShow) {
         ...
         function updateSelAllCheckbox(ancestor) {
            ...
            [].forEach.call(inputs, function(inp) {
               if (inp === selector) {} else if (inp.indeterminate) {
                  selectedAll = false;
                  selectedNone = false;
               } else if (inp.checked) {
                  selectedNone = false;
         ...
         var labelType;
         if (tree.layer) {
            if ((this.options.labelIsSelector === 'both') || (overlay && this.options.labelIsSelector === 'overlay') || (!overlay && this.options.labelIsSelector === 'base')) {
               labelType = 'label'
            } else {
               labelType = 'span'
            }
